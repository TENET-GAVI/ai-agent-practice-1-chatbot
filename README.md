...
该页面不是静态说明页，而是可以输入消息并获得回复的提示词智能体。它通过 Puter.js 在前端调用 `puter.ai.chat([messages])`，会把最近对话历史一起发送给模型，从而根据上下文继续聊天；如果在线 AI 临时不可用，会自动切到本地上下文兜底回复。

## 文件清单

- `index.html`：可聊天的智能体页面。
- `styles.css`：页面样式。
- `agent.js`：智能体提示词、Puter.js AI 对话调用、上下文历史和本地兜底逻辑。
